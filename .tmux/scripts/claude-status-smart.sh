...
detect_claude_status() {
    ...
    if echo "$recent_output" | grep -qE '[✢✻*✽✶]\s*(Thinking|Ruminating|Finagling|Processing|Working|Sparkling|Designing|Percolating|Computing|Ideating|Herding)…\s*\([0-9]+s\s*[·•]\s*([↓↑⚒]\s*)?[0-9,.]+[km]?\s*tokens\s*[·•]\s*(esc to interrupt|interrupt)\)'; then
    ...
    elif echo "$recent_output" | grep -qE '(Finagling|Ruminating|Thinking|Processing|Working|Sparkling|Designing|Percolating|Computing|Ideating|Herding)\.\.\.|[✢✻*✽✶]\s*(Finagling|Ruminating|Thinking|Processing|Working|Sparkling|Designing|Percolating|Computing|Ideating|Herding)'; then
    ...
    # 2. WAITING: ユーザー入力が必要な状態（Busy処理完了後にチェック）
         ...
         ! echo "$recent_output" | grep -qE '(Finagling|Ruminating|Thinking|Processing|Working|Sparkling|Designing|Percolating|Computing|Ideating|Herding|tokens.*interrupt|✅.*完了)'; then
        status_icon="⌛"  # 選択肢プロンプト
         ...
         ! echo "$recent_output" | grep -qE '[✢✻*✽✶]\s*(Thinking|Ruminating|Finagling|Processing|Working|Sparkling|Designing|Percolating|Computing|Ideating|Herding)'; then
        status_icon="⌛"  # プラン承認待ち
...
main() {
    ...
    case "$mode" in
        ...
        "test")
            ...
            if echo "$recent_output" | grep -qE '(Finagling|Ruminating|Thinking|Processing|Working|Sparkling|Designing|Percolating|Computing|Ideating|Herding)\.\.\.|[✢✻*✽✶]\s*(Finagling|Ruminating|Thinking|Processing|Working|Sparkling|Designing|Percolating|Computing|Ideating|Herding)'; then
