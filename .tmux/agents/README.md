...
一覧は `対象 / 状態 / リポジトリ / ブランチ / worktree / pane` の順で表示する。cwd は詳細表示に残し、
一覧では状態の隣に tmux の window（`番号:名前`）と pane 番号を出し、続けてリポジトリ名・ブランチ名・worktree名へ分ける。列幅は全行の内容幅から自動で決め（余白は右端へ）、狭いpopupでは worktree → リポジトリ（親行と重複）を先に隠し、それでも足りなければ長い列から切り詰める。

`main`、`master` などの基準ブランチは緑、`feature/`・`feat/` はシアン、`fix/`・`bugfix/` は黄、
`hotfix/` は赤、`release/` は紫で表示する。状態も色と `●` のラベルで表すため、色を使えない端末でも判別できる。
`◆` はリポジトリ、`⌂` は基準worktree、`⌘` は追加worktree、`⚙` は自動生成したworktreeを示す。
...
| キー | 操作 |
|---|---|
| `Ctrl+n` / `Ctrl+p`、↑↓ | 次 / 前の行を選択 |
| `Ctrl+f` / `Ctrl+b`、←→ | 展開して子へ / 折りたたんで親へ |
| `Ctrl+l` | 画面を再表示 |
