...
## Liveness Check（ポーリング）

`polling_monitor.sh` は Hooks の補助として軽量な Liveness Check のみを行う。
`capture-pane` によるステータス判定は廃止済み。

### 役割

1. **生存確認**: 登録済みペインの `pane_title` に "Claude Code" が含まれるかチェック
2. **クリーンアップ**: CC が終了したペインの tmux option を削除、アイコンを再集約
3. **自動登録**: Hooks 未発火の CC ペインを Idle で登録（セーフティネット）

```
登録済みペイン + title に "Claude Code" → 何もしない（Hooks が管理）
登録済みペイン + title に "Claude Code" なし → ステータスクリア
未登録ペイン + title に "Claude Code" → Idle で自動登録
```
