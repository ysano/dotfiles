...
gitコミットとLinearタスク更新から日次スタンドアップレポートを生成し、チームメンバーがスタンドアップミーティングの準備をサポートします。
...
### Linear接続なし
```
"Linear MCPサーバーが接続されていません。gitとGitHubデータのみからレポートを生成します。

完全な機能を有効にするには:
1. Linear MCPをインストール: npm install -g @modelcontextprotocol/server-linear
2. APIキーで設定
3. Linearを接続して再起動
