...
| 7 | version skew | 走行中 claude セッションの起動時刻 < `@anthropic-ai/claude-code/package.json` の mtime（= 起動後に auto-update がディスクを更新）で ⚠️。プロセスはメモリ内の旧コードのまま、新 spawn の cc-daemon は新コードで起動するため世代差が配送障害の温床になる（2026-07-09 に5日差の skew を実測） | 区切りで `/exit` → `claude -c`（会話は resume で復元） |
